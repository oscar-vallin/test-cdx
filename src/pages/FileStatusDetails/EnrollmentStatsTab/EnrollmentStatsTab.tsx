...
const EnrollmentStatsTab = ({ packet }: EnrollmentStatsTabProps): ReactElement => {
  const insuredStats =
    packet?.enrollmentStats?.planInsuredStat
      ?.filter((itm) => itm)
      ?.map((plan) => ({
  ...
  const excludedInsuredStats =
    packet?.enrollmentStats?.excludedPlanInsuredStat
      ?.filter((itm) => itm)
      ?.map((plan) => ({
        planCode: plan?.planCode,
        activeSubscribers: plan?.subscribers?.active?.value || 0,
          ...
          <Stack.Item>
            <Card>
              <Stack horizontal tokens={{ childrenGap: 10, padding: 10 }}>
                <Stack.Item grow>
                  <Text>Included Subscribers / Enrollments</Text>
                </Stack.Item>
                <Stack.Item grow>
        ...
        {excludedInsuredStats?.length > 0 && (
          <Stack.Item>
            <Card>
              <Stack horizontal tokens={{ childrenGap: 10, padding: 10 }}>
                <Stack.Item grow>
                  <Text>Excluded Subscribers / Enrollments</Text>
                </Stack.Item>
                <Stack.Item grow>
                  <Text>Subscribers</Text>
                </Stack.Item>

...
async function orgQuickSearch(client: ApolloClient<object>, handleError: (error?: any) => void, searchText: string, orgOwnerSid: string): Promise<ITag[]> {
  let orgs: ITag[] = [];
  await client.query({
    errorPolicy: 'all',
    variables: {
...
async function vendorQuickSearch(client: ApolloClient<object>, handleError: (error?: any) => void, searchText: string, orgOwnerSid: string): Promise<ITag[]> {
  let orgs: ITag[] = [];
  await client.query({
    errorPolicy: 'all',
    variables: {

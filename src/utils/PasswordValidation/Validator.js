...
class PasswordRulesValidator {
  ...
  static countExpectations = (rule) => {
    ...
    const expectations = data.rules.map(PasswordRulesValidator.countExpectations);

    return _.chain([data.expectation, ...expectations])
      .flattenDeep()
      .sum()
      .value();
  };
  ...
  static validate(value, rules) {
    ...
    return [...Array.from(new Set(manualValidations)), ...validator.validate(value, { list: true })];
  }
    ...
    return data.rules.map((ruleSet) => {
      ...
      if (item.rules) {
        return {
          ...
          isValid: PasswordRulesValidator.getValidationStatus(item).reduce(
            (validations, isValid) => [...validations, isValid],
            []
          ),
        };
      }
